...
let
  ...
  clevisDatasets = lib.attrNames (lib.filterAttrs (device: _: lib.any (e: e.fsType == "zfs" && (fsNeededForBoot e) && (e.device == device || lib.hasPrefix "${device}/" e.device)) config.system.build.fileSystems) config.boot.initrd.clevis.devices);
  ...
  datasetToPool = x: lib.elemAt (splitString "/" x) 0;
  ...
  rootPools = lib.unique (map fsToPool (lib.filter fsNeededForBoot zfsFilesystems));
  ...
  getPoolMounts = prefix: pool:
    let
      ...
      mountPoint = fs: lib.escapeSystemdPath (prefix + (lib.removeSuffix "/" fs.mountPoint));
  ...
  createImportService = { pool, systemd, force, prefix ? "" }:
    lib.nameValuePair "zfs-import-${pool}" {
      ...
      in (importLib {
        ...
        if poolImported "${pool}"; then
        ${lib.optionalString config.boot.initrd.clevis.enable (lib.concatMapStringsSep "\n" (elem: "clevis decrypt < /etc/clevis/${elem}.jwe | zfs load-key ${elem} || true ") (lib.filter (p: (lib.elemAt (splitString "/" p) 0) == pool) clevisDatasets))}
      ...
      boot.initrd = lib.mkIf inInitrd {
        ...
        postResumeCommands = lib.mkIf (!config.boot.initrd.systemd.enable) (lib.concatStringsSep "\n" ([''
          ...
          ''] ++ [(importLib {
            ...
            ${lib.optionalString config.boot.initrd.clevis.enable (lib.concatMapStringsSep "\n" (elem: "clevis decrypt < /etc/clevis/${elem}.jwe | zfs load-key ${elem}") (lib.filter (p: (lib.elemAt (splitString "/" p) 0) == pool) clevisDatasets))}
