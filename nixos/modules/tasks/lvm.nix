...
in {
  options.services.lvm = {
    enable = lib.mkEnableOption "lvm2" // {
      default = true;
    };
    ...
    package = lib.mkOption {
      ...
      default = pkgs.lvm2;
      internal = true;
      defaultText = lib.literalExpression "pkgs.lvm2";
      description = ''
    ...
    dmeventd.enable = lib.mkEnableOption "the LVM dmevent daemon";
    boot.thin.enable = lib.mkEnableOption "support for booting from ThinLVs";
    boot.vdo.enable = lib.mkEnableOption "support for booting from VDOLVs";
  };

  options.boot.initrd.services.lvm.enable = lib.mkEnableOption "booting from LVM2 in the initrd" // {
    description = ''
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf cfg.dmeventd.enable {
      ...
      '';
      services.lvm.package = lib.mkDefault pkgs.lvm2_dmeventd;
    })
    (lib.mkIf cfg.boot.thin.enable {
      boot.initrd = {
      ...
      environment.etc."lvm/lvm.conf".text = lib.concatMapStringsSep "\n"
    ...
    (lib.mkIf cfg.boot.vdo.enable {
      ...
      services.lvm.package = lib.mkOverride 999 pkgs.lvm2_vdo;  # this overrides lib.mkDefault

      environment.systemPackages = [ pkgs.vdo ];
    })
    (lib.mkIf (cfg.dmeventd.enable || cfg.boot.thin.enable) {
      boot.initrd.systemd.contents."/etc/lvm/lvm.conf".text = lib.optionalString (config.boot.initrd.services.lvm.enable && cfg.boot.thin.enable) (lib.concatMapStringsSep "\n"
          (bin: "global/${bin}_executable = /bin/${bin}")
          [ "thin_check" "thin_dump" "thin_repair" "cache_check" "cache_dump" "cache_repair" ]
        ) + "\n" + lib.optionalString cfg.dmeventd.enable ''
      ...
      boot.initrd.preLVMCommands = lib.mkIf (!config.boot.initrd.systemd.enable) ''
          ...
          ${lib.optionalString cfg.boot.thin.enable (
