...
let
  ...
  fileSystems' = lib.lists.toposort utils.fsBefore (lib.attrValues config.fileSystems);
  ...
  config = {
    ...
    in [
      ...
      { assertion = ! (lib.any notAutoResizable fileSystems);
        message = let
          fs = head (lib.filter notAutoResizable fileSystems);
        in ''
      ...
      }
      {
        ...
        message = let
          fs = head (lib.filter (fs: fs.formatOptions != null) fileSystems);
        in ''
    ...
    system.build.earlyMountScript = makeSpecialMounts (lib.lists.toposort fsBefore (lib.attrValues config.boot.specialFileSystems)).result;
