...
let
  ...
  interfaceRoutes = i:
    i.ipv4.routes
    ++ lib.optionals cfg.enableIPv6 i.ipv6.routes;
  ...
  defaultGateways = lib.mkMerge (lib.forEach [ cfg.defaultGateway cfg.defaultGateway6 ] (gateway:
  ...
  genericDhcpNetworks = initrd: lib.mkIf cfg.useDHCP {
  ...
  interfaceNetworks = lib.mkMerge (lib.forEach interfaces (i: {
  ...
  bridgeNetworks = lib.mkMerge (lib.flip lib.mapAttrsToList cfg.bridges (name: bridge: {
    netdevs."40-${name}" = {
      netdevConfig = {
        Name = name;
        Kind = "bridge";
    ...
    networks = lib.listToAttrs (forEach bridge.interfaces (bi:
  ...
  vlanNetworks = lib.mkMerge (lib.flip lib.mapAttrsToList cfg.vlans (name: vlan: {
    netdevs."40-${name}" = {
      netdevConfig = {
        Name = name;
        Kind = "vlan";
...

{
  ...
  (lib.mkIf cfg.useNetworkd {
    ...
    } ] ++ lib.flip lib.mapAttrsToList cfg.bridges (n: { rstp, ... }: {
      assertion = !rstp;
      message = "networking.bridges.${n}.rstp is not supported by networkd.";
    }) ++ lib.flip lib.mapAttrsToList cfg.fooOverUDP (n: { local, ... }: {
    ...
    systemd.network =
      ...
      interfaceNetworks
      bridgeNetworks
      (lib.mkMerge (lib.flip lib.mapAttrsToList cfg.bonds (name: bond: {
        netdevs."40-${name}" = {
          netdevConfig = {
            Name = name;
            Kind = "bond";
          ...
          bondConfig = let
            ...
            assertNoUnknownOption = let
              knownOptions = lib.flatten (lib.mapAttrsToList (_: kOpts: kOpts.optNames)
                                                     driverOptionMapping);
              ...
              in assert all (driverOpt: assertTrace
                               (lib.elem driverOpt (knownOptions ++ unknownOptions))
            ...
            filterSystemdOptions = lib.filterAttrs (sysDOpt: kOpts:
                                     lib.any (kOpt: do ? ${kOpt}) kOpts.optNames);
            # build final set of systemd options to bond values
            buildOptionSet = lib.mapAttrs (_: kOpts: with kOpts;
                               ...
                               head (map (optN: valTransform (do.${optN}))
                                 ...
                                 (lib.filter (o: do ? ${o}) optNames)));
            in seq assertNoUnknownOption
                   (buildOptionSet (filterSystemdOptions driverOptionMapping));

        };

        networks = lib.listToAttrs (forEach bond.interfaces (bi:
          lib.nameValuePair "40-${bi}" {
            DHCP = lib.mkOverride 0 (dhcpStr false);
            networkConfig.Bond = name;
          }));
      })))
      (lib.mkMerge (lib.flip lib.mapAttrsToList cfg.macvlans (name: macvlan: {
        netdevs."40-${name}" = {
          netdevConfig = {
            Name = name;
            Kind = "macvlan";
      ...
      (lib.mkMerge (lib.flip lib.mapAttrsToList cfg.fooOverUDP (name: fou: {
        netdevs."40-${name}" = {
          netdevConfig = {
            Name = name;
            Kind = "fou";
      ...
      (lib.mkMerge (lib.flip lib.mapAttrsToList cfg.sits (name: sit: {
        netdevs."40-${name}" = {
          netdevConfig = {
            Name = name;
            Kind = "sit";
      ...
      (lib.mkMerge (lib.flip lib.mapAttrsToList cfg.greTunnels (name: gre: {
        netdevs."40-${name}" = {
          netdevConfig = {
            Name = name;
            Kind = gre.type;
    ...
    systemd.services = let
      ...
      subsystemDevice = interface:
        "sys-subsystem-net-devices-${lib.utils.escapeSystemdPath interface}.device";
      # support for creating openvswitch switches
      createVswitchDevice = n: v: lib.nameValuePair "${n}-netdev"
          (let
            deps = map subsystemDevice (lib.attrNames (lib.filterAttrs (_: config: config.type != "internal") v.interfaces));
          ...
          { description = "Open vSwitch Interface ${n}";
            ...
            script = ''
              ...
              ovs-vsctl ${lib.concatStrings (lib.mapAttrsToList (name: config: " -- add-port ${n} ${name}" + lib.optionalString (config.vlan != null) " tag=${toString config.vlan}") v.interfaces)} \
                ...
                ${lib.concatMapStrings (x: " -- " + x) (splitString "\n" v.extraOvsctlCmds)}
    ...
    in lib.mapAttrs' createVswitchDevice cfg.vswitches
      // {
