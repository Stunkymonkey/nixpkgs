...
}:
{
  ...
  config =
    ...
    lib.mkIf cfg.enable {
      ...
      networking = lib.mkIf (!cfg.manageNetwork) {
        useDHCP = false;
        useHostResolvConf = false;
        useNetworkd = true;
        # pick up hostname from /etc/hostname generated by proxmox
        hostName = lib.mkIf (!cfg.manageHostName) (lib.mkForce "");
      };

      # unprivileged LXCs can't set net.ipv4.ping_group_range
      security.wrappers.ping = lib.mkIf (!cfg.privileged) {
        owner = "root";
        group = "root";
      ...
      systemd = {
        mounts = lib.mkIf (!cfg.privileged) [
          {
            enable = false;
