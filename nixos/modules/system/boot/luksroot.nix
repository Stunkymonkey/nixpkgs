  ...
  options = {
    ...
    boot.initrd.luks.mitigateDMAAttacks = lib.mkOption {
      ...
      description = ''
        ...
        access to lib.any machine with PCMCIA, ExpressCard, ThunderBolt or FireWire port.
        More information is available at <https://en.wikipedia.org/wiki/DMA_attack>.

        This option blacklib.lists FireWire drivers, but doesn't remove them. You can manually
    ...
    boot.initrd.luks.reusePassphrases = lib.mkOption {
      ...
      default = true;
      description = ''
        When opening a new LUKS device try reusing lib.last successful
        passphrase.
        ...
        { config, name, ... }: { options = {
          ...
          crypttabExtraOpts = lib.mkOption {
            ...
            description = ''
              ...
              Extra options to append to the lib.last column of the generated crypttab file.
            '';
          };
        };

        config = lib.mkIf (clevis.enable && (lib.hasAttr name clevis.devices)) {
      ...
      lib.mkIf (clevis.enable && systemd.enable) (
        (lib.mapAttrs'
          (name: _: lib.nameValuePair "cryptsetup-clevis-${name}" {
            wantedBy = [ "systemd-cryptsetup@${utils.lib.escapeSystemdPath name}.service" ];
            before = [
              "systemd-cryptsetup@${utils.lib.escapeSystemdPath name}.service"
