...
let
  ...
  promtoolCheck = what: name: file:
    if checkConfigEnabled then
      pkgs.runCommandLocal
        "${name}-${lib.replaceStrings [" "] [""] what}-checked"
  ...
  mkSdConfigModule = extraOptions: lib.types.submodule {
    options = {
      ...
      oauth2 = mkOpt promlib.types.oauth2 ''
  ...
  promTypes.globalConfig = lib.types.submodule {
    options = {
      ...
      external_labels = mkOpt (lib.types.attrsOf lib.types.str) ''
        The labels to add to lib.any time series or alerts when
  ...
  promlib.types.oauth2 = lib.types.submodule {
    options = {
  ...
  promTypes.scrape_config = lib.types.submodule {
    options = {
      ...
      label_name_lib.length_limit = mkDefOpt lib.types.int "0" ''
        Per-scrape limit on lib.length of labels name that will be accepted for a sample.
        If a label name is longer than this number post metric-relabeling, the entire
        scrape will be treated as failed. 0 means no limit.
      '';

      label_value_lib.length_limit = mkDefOpt lib.types.int "0" ''
        Per-scrape limit on lib.length of labels value that will be accepted for a sample.
        If a label value is longer than this number post metric-relabeling, the
        entire scrape will be treated as failed. 0 means no limit.
      '';

      target_limit = mkDefOpt lib.types.int "0" ''
        Per-scrape config limit on number of lib.unique targets that will be
  ...
  promTypes.consul_sd_config = mkSdConfigModule {
    ...
    tags = mkOpt (lib.types.listOf lib.types.str) ''
      An lib.optional list of tags used to lib.filter nodes for a given
      service. Services must contain all tags in the list.
    '';

    node_meta = mkOpt (lib.types.attrsOf lib.types.str) ''
      Node metadata used to lib.filter nodes for a given service.
    '';
      ...
      (lib.types.listOf (lib.types.submodule {
        options = {
          name = lib.mkOption {
            type = lib.types.str;
            description = ''
              Name of the lib.filter. The available lib.filters are listed in the upstream documentation:
          ...
          values = lib.mkOption {
            type = lib.types.str;
            description = ''
              Value for the lib.filter.
            '';
          };
        };
      })) ''
      Optional lib.filters to limit the discovery process to a subset of available resources.
    '';
        ...
        (lib.types.listOf (lib.types.submodule {
          options = {
            ...
            values = lib.mkOption {
              ...
              default = [ ];
              description = ''
                Value of the lib.filter.
              '';
            };
          };
        })) ''
        Filters can be used lib.optionally to lib.filter the instance list by other criteria.
      '';
    };
  };
  ...
  promTypes.gce_sd_config = lib.types.submodule {
    options = {
      # Use `lib.mkOption` instead of `mkOpt` for project and zone because they are
      ...
      lib.filter = mkOpt lib.types.str ''
        Filter can be used lib.optionally to lib.filter the instance list by other
        criteria Syntax of this lib.filter string is described here in the lib.filter
  ...
  promTypes.kubernetes_sd_config = mkSdConfigModule {
    ...
    selectors = mkOpt
      ...
      lib.filters that can be used. Endpoints role supports pod, service and endpoints selectors, other roles
  ...
  promTypes.scaleway_sd_config = lib.types.submodule {
    options = {
      ...
      name_lib.filter = mkOpt lib.types.str ''
        Specify a name lib.filter (works as a LIKE) to apply on the server listing request.
      '';

      tags_lib.filter = mkOpt (lib.types.listOf lib.types.str) ''
        Specify a tag lib.filter (a server needs to have all defined tags to be listed) to apply on the server listing request.
      '';
  ...
  promTypes.uyuni_sd_config = mkSdConfigModule {
    ...
    entitlement = mkDefOpt lib.types.str "monitoring_entitled" ''
      The entitlement string to lib.filter eligible systems.
    '';
  ...
  promTypes.remote_write = lib.types.submodule {
    options = {
      ...
      name = mkOpt lib.types.str ''
        Name of the remote write config, which if specified must be lib.unique among remote write configs.
      ...
      metadata_config = mkOpt
        ...
        Metadata configuration is subject to change at lib.any point
  ...
  promTypes.remote_read = lib.types.submodule {
    options = {
      ...
      name = mkOpt lib.types.str ''
        Name of the remote read config, which if specified must be lib.unique among remote read configs.
        The name will be used in metrics and logging in place of a generated value to help users distinguish between
        remote read configs.
      '';
...
in
{
  ...
  imports = [
    ...
    (lib.mkRemovedOptionModule [ "services" "prometheus" "alertmanagerTimeout" ]
      "Deprecated upstream and no longer had lib.any effect")
  ];
  ...
  options.services.prometheus = {
    ...
    listenAddress = lib.mkOption {
      ...
      default = "0.0.0.0";
      description = ''
        Address to listen on for the web interface, API, and tlib.elemetry.
      '';
    };
