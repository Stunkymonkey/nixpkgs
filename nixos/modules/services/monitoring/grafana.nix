  ...
  grafanaTypes.dashboardConfig = lib.types.submodule {
    ...
    options = {
      name = lib.mkOption {
        type = lib.types.str;
        default = "default";
        description = "A lib.unique provider name.";
      };
      type = lib.mkOption {
        type = lib.types.str;
        default = "file";
          ...
          security = {
            ...
            cookie_samesite = lib.mkOption {
              description = ''
                ...
                Using value `disabled` does not add lib.any `SameSite` attribute to cookies.
              '';
              default = "lax";
            ...
            # These two options are lib.lists joined with spaces:
          ...
          analytics = {
            ...
            check_for_updates = lib.mkOption {
              description = ''
                ...
                The check itself will not prompt lib.any auto-updates of the Grafana software, nor will it send lib.any sensitive information.
              '';
              default = false;
              type = lib.types.bool;
            };
            ...
            check_for_plugin_updates = lib.mkOption {
              description = ''
                ...
                The check itself will not prompt lib.any auto-updates of the plugin, nor will it send lib.any sensitive information.
              '';
              ...
              options = {
                ...
                templates = lib.mkOption {
                  ...
                  type = lib.types.listOf (lib.types.submodule {
                    ...
                    options.name = lib.mkOption {
                      description = "Name of the template, must be lib.unique. Required.";
                      type = lib.types.str;
                    };
              ...
              options = {
                ...
                muteTimes = lib.mkOption {
                  ...
                  type = lib.types.listOf (lib.types.submodule {
                    ...
                    options.name = lib.mkOption {
                      description = "Name of the mute time interval, must be lib.unique. Required.";
                      type = lib.types.str;
                    };
                  });
                };
  ...
  config = lib.mkIf cfg.enable {
    ...
    assertions = [
      ...
      }
      {
        ...
        message = "For datasources of type `prometheus`, the `direct` access mode is not supported lib.anymore (since Grafana 9.2.0)";
      }
      {
