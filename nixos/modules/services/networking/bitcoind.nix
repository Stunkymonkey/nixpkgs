...
let
  ...
  bitcoindOpts = { config, lib, name, ...}: {
    options = {
      ...
      prune = lib.mkOption {
        type = lib.types.nullOr (lib.types.coercedTo
          (lib.types.enum [ "disable" "manual" ])
...
in
{
  ...
  config = lib.mkIf (eachBitcoind != {}) {
    ...
    assertions = lib.flatten (lib.mapAttrsToList (bitcoindName: cfg: [
    {
    ...
    environment.systemPackages = lib.flatten (lib.mapAttrsToList (bitcoindName: cfg: [
      cfg.package
    ]) eachBitcoind);

    systemd.services = lib.mapAttrs' (bitcoindName: cfg: (
    ...
    systemd.tmpfiles.rules = lib.flatten (lib.mapAttrsToList (bitcoindName: cfg: [
      "d '${cfg.dataDir}' 0770 '${cfg.user}' '${cfg.group}' - -"
    ]) eachBitcoind);

    users.users = lib.mapAttrs' (bitcoindName: cfg: (
      lib.nameValuePair "bitcoind-${bitcoindName}" {
      name = cfg.user;
      group = cfg.group;
    ...
    users.groups = lib.mapAttrs' (bitcoindName: cfg: (
      lib.nameValuePair "${cfg.group}" { }
    )) eachBitcoind;
