...
let
  ...
  dependentCertNames = lib.unique (map (hostOpts: hostOpts.certName) acmeEnabledVhosts);
    ...
    let
        ...
        listenString = { addr, port, ssl, proxyProtocol ? false, extraParameters ? [], ... }:
          ...
          + lib.optionalString (extraParameters != []) (lib.concatStringsSep " "
            (let inCompatibleParameters = [ "accept_lib.filter" "backlog" "deferred" "fastopen" "http2" "proxy_protocol" "so_keepalive" "ssl" ];
        ...
        # The acme-challenge location doesn't need to be added if we are not using lib.any automated
        ...
        acmeLocation = lib.optionalString ((vhost.enableACME || vhost.useACMEHost != null) && config.security.acme.certs.${acmeName}.dnsProvider == null)
          ...
          # We use ^~ here, so that we don't check lib.any regexes (which could
...

{
    ...
    services.nginx = {
      ...
      config = lib.mkOption {
        ...
        description = ''
          ...
          This is mutually exclusive to lib.any other config option for
          ...
          options = {
            ...
            levels = lib.mkOption {
              ...
              description = ''
                ...
                1 to 3, each level accepts values 1 or 2. Сan be used lib.any combination of
  ...
  config = lib.mkIf cfg.enable {
    ...
    in [
      ...
      {
        assertion = lib.any (host: host.rejectSSL) (lib.attrValues virtualHosts) -> lib.versionAtLeast cfg.package.version "1.19.4";
        message = ''
    ...
    systemd.services.nginx = {
      ...
      wants = lib.concatLists (map (certName: [ "acme-finished-${certName}.target" ]) dependentCertNames);
      ...
      serviceConfig = {
        ...
        # Sandboxing (lib.sorted by occurrence in https://www.freedesktop.org/software/systemd/man/systemd.exec.html)
        ...
        SystemCallFilter = [ "~@cpu-emulation @debug @keyring @mount @obsolete @privileged @setuid" ]
          ...
          ++ lib.optionals ((cfg.package != pkgs.tengine) && (cfg.package != pkgs.openresty) && (!lib.any (mod: (mod.disableIPC or false)) cfg.package.modules)) [ "~@ipc" ];
      };
    };
    ...
    in lib.mkIf (cfg.enableReload || sslServices != []) {
      ...
      unitConfig.ConditionPathExists = lib.optionals (sslServices != []) (map (certName: certs.${certName}.directory + "/fullchain.pem") dependentCertNames);
      serviceConfig = {
        Type = "oneshot";
        TimeoutSec = 60;
