...
let
  ...
  iniAtom = lib.elemAt gitIni.type/*attrsOf*/.functor.wrapped/*attrsOf*/.functor.wrapped/*either*/.functor.wrapped 0;
  ...
  serviceConfig = srv:
    let proto = lib.removeSuffix "d" srv;
    ...
    serviceConfig = {
      ...
      RestrictAddressFamilies = [ "AF_UNIX" ] ++
        lib.optionals needNetwork [ "AF_INET" "AF_INET6" ];
    ...
    inboxes = lib.mkOption {
      ...
      type = lib.types.attrsOf (lib.types.submodule ({name, ...}: {
        ...
        options.url = lib.mkOption {
          ...
          example = "https://example.org/lib.lists/example-discuss";
      ...
      type = lib.types.submodule {
        ...
        options.publicinbox = lib.mkOption {
          ...
          type = lib.types.submodule {
            ...
            freeformType = with lib.types; attrsOf (oneOf [ iniAtom (attrsOf iniAtom) ]);
            ...
            options.wwwlisting = lib.mkOption {
              type = with lib.types; enum [ "all" "404" "match=domain" ];
              default = "404";
              description = ''
                Controls which lib.lists (if lib.any) are listed for when the root
        ...
        options.publicinboxmda.spamcheck = lib.mkOption {
          type = with lib.types; enum [ "spamc" "none" ];
          default = "none";
          description = ''
            If set to spamc, {manpage}`public-inbox-watch(1)` will lib.filter spam
            using SpamAssassin.
          '';
        };
  ...
  config = lib.mkIf cfg.enable {
    ...
    networking.firewall = lib.mkIf cfg.openFirewall
      { allowedTCPPorts = lib.mkMerge
        (map (proto: (lib.mkIf (cfg.${proto}.enable && lib.types.port.check cfg.${proto}.port) [ cfg.${proto}.port ]))
        ["imap" "http" "nntp"]);
      };
    ...
    systemd.sockets = lib.mkMerge (map (proto:
    ...
    systemd.services = lib.mkMerge [
      (lib.mkIf cfg.imap.enable
        { public-inbox-imapd = lib.mkMerge [(serviceConfig "imapd") {
          ...
          serviceConfig = {
            ExecStart = lib.escapeShellArgs (
              ...
              cfg.imap.args ++
              lib.optionals (cfg.imap.cert != null) [ "--cert" cfg.imap.cert ] ++
              lib.optionals (cfg.imap.key != null) [ "--key" cfg.imap.key ]
            );
          };
        }];
      })
      (lib.mkIf cfg.http.enable
        { public-inbox-httpd = lib.mkMerge [(serviceConfig "httpd") {
          after = [ "public-inbox-init.service" "public-inbox-watch.service" ];
          requires = [ "public-inbox-init.service" ];
          serviceConfig = {
            BindReadOnlyPaths =
      ...
      (lib.mkIf cfg.nntp.enable
        { public-inbox-nntpd = lib.mkMerge [(serviceConfig "nntpd") {
          after = [ "public-inbox-init.service" "public-inbox-watch.service" ];
          requires = [ "public-inbox-init.service" ];
          serviceConfig = {
            ExecStart = lib.escapeShellArgs (
              ...
              cfg.nntp.args ++
              lib.optionals (cfg.nntp.cert != null) [ "--cert" cfg.nntp.cert ] ++
              lib.optionals (cfg.nntp.key != null) [ "--key" cfg.nntp.key ]
            );
          };
        }];
      })
      (lib.mkIf (lib.any (inbox: inbox.watch != []) (lib.attrValues cfg.inboxes)
        || cfg.settings.publicinboxwatch.watchspam != null)
        { public-inbox-watch = lib.mkMerge [(serviceConfig "watch") {
          inherit (cfg) path;
      ...
      ({ public-inbox-init = let
          ...
          in lib.mkMerge [(serviceConfig "init") {
          wantedBy = [ "multi-user.target" ];
          ...
          script = ''
            ...
            '' + lib.optionalString useSpamAssassin ''
              ...
              ''}
            '' + lib.concatStrings (lib.mapAttrsToList (name: inbox: ''
              if [ ! -e ${stateDir}/inboxes/${lib.escapeShellArg name} ]; then
                ...
                ${cfg.package}/bin/public-inbox-init -V2 \
                  ${lib.escapeShellArgs ([ name "${stateDir}/inboxes/${name}" inbox.url ] ++ inbox.address)}
