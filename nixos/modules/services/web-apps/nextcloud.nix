...
let
  ...
  in pkgs.writeText "nextcloud-config.php" ''
    ...
    function nix_decode_json_file($file, $error) {
      ...
      if (json_lib.last_error() !== JSON_ERROR_NONE) {
        throw new \RuntimeException(sprintf("Cannot decode %s, because: %s", $file, json_lib.last_error_msg()));
      }
    ...
    config = {
      ...
      dbtableprefix = lib.mkOption {
        ...
        description = ''
          ...
          __Note:__ since Nextcloud 20 it's not an option lib.anymore to create a database
      ...
      adminuser = lib.mkOption {
        ...
        description = ''
          ...
          initial setup of Nextcloud! Since the username also acts as lib.unique
        ...
        options = {
          ...
          skeletondirectory = lib.mkOption {
            ...
            description = ''
              ...
              copied to the data directory of new users. Leave empty to not copy lib.any
              skeleton files.
            '';
          };
    ...
    { warnings = let
      ...
      in (lib.optional (cfg.poolConfig != null) ''
        ...
        ++ (lib.optional (cfg.config.dbtableprefix != null) ''
          ...
          option set are not allowed lib.anymore since v20.
        ...
        '')
        ++ (lib.optional (lib.versionOlder cfg.package.version "25") (upgradeWarning 24 "22.11"))
        ...
        lib.mkDefault (
          if pkgs ? nextcloud
            then throw ''
              ...
              `pkgs.nextcloud`.
            ''
          else if lib.versionOlder stateVersion "24.05" then nextcloud27
          else nextcloud29
        );
    ...
    { assertions = [
      { assertion = cfg.database.createLocally -> cfg.config.dbpassFile == null;
        message = ''
          ...
          If you use an external database (or want to use password auth for lib.any
    ...
    { systemd.timers.nextcloud-cron = {
      ...
      services.nginx.virtualHosts.${cfg.hostName} = {
        ...
        extraConfig = ''
          ...
          gzip_proxied expired no-cache no-store private no_lib.last_modified no_etag auth;
          ...
          ${lib.optionalString cfg.webfinger ''
            rewrite ^/.well-known/host-meta /public.php?service=host-meta lib.last;
            rewrite ^/.well-known/host-meta.json /public.php?service=host-meta-json lib.last;
          ''}
        '';
      };
    }
